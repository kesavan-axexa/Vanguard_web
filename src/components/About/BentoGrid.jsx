        ...
        {items.map((item, index) => (
          <div
            ...
            className={`group relative p-4 rounded-xl overflow-hidden transition-all duration-300
                            ...
                            hover:shadow-lg hover:-translate-y-1 will-change-transform shadow-lg
                            ${item.colSpan === 2 ? "md:col-span-2" : ""}
                        `}
          >
            <div className="relative flex flex-col space-y-3">
              <div className="flex items-center justify-between">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-black/5 transition-all duration-300">
                  {item.icon}
                </div>
                {/* <span className="text-xs font-medium px-2 py-1 rounded-lg backdrop-blur-xs bg-black/5 text-gray-600  transition-colors duration-300">
                                    {item.status || "Active"}
                                </span> */}
              </div>
              ...
              <div className="flex items-center justify-between mt-2">
                <div className="flex items-center space-x-2 text-xs text-gray-500 d">
                  {item.tags?.map((tag, i) => (
                    <span
                      key={i}
